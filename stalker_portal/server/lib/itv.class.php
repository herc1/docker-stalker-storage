...
class Itv extends AjaxResponse implements \Stalker\Lib\StbApi\Itv {
    ...
    public static function getFilteredUserChannelsIds() {
        ...
        $disable_channel_filter_for_logins = Config::getSafe('disable_channel_filter_for_logins', array());

        $mac = Stb::getInstance()->mac;
        $login = Stb::getInstance()->getParam('login');

        $user_links = array_filter($all_links, function ($link) use ($user_agent, $disable_channel_filter_for_macs, $mac, $disable_channel_filter_for_logins, $login) {
            return $link['user_agent_filter'] == '' || in_array($mac, $disable_channel_filter_for_macs) || $login && in_array($login, $disable_channel_filter_for_logins) || preg_match("/" . $link['user_agent_filter'] . "/", $user_agent);
        });
    ...
    public function getUrlsMapForChannels($ch_ids, $force_ch_link_check = false) {
        ...
        foreach ($grouped_links as $ch_id => $links) {
            ...
            $user_channel_links = array_filter($links, function ($link) use ($user_agent, $disable_channel_filter_for_macs, $mac, $disable_channel_filter_for_logins, $login) {
                return in_array($mac, $disable_channel_filter_for_macs) || $login && in_array($login, $disable_channel_filter_for_logins) || $link['user_agent_filter'] != '' && preg_match("/" . $link['user_agent_filter'] . "/", $user_agent);
            });
    ...
    public static function getUrlsForChannel($ch_id, $for_pvr = false, $force_ch_link_check = false) {
        ...
        $user_channel_links = array_filter($channel_links, function ($link) use ($user_agent, $disable_channel_filter_for_macs, $mac, $disable_channel_filter_for_logins, $login) {
            return in_array($mac, $disable_channel_filter_for_macs) || $login && in_array($login, $disable_channel_filter_for_logins) || $link['user_agent_filter'] != '' && preg_match("/" . $link['user_agent_filter'] . "/", $user_agent);
        });
