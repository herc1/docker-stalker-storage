...
//        CloseModalBox();
    ...
    $(document).on('click', "#attribute_set li:first-of-type label", function (e) {
        ...
        $("#attribute_set input[type='checkbox']:not(:first):not(:disabled)").each(function(index){
		...
		$("table[id^='datatable']").DataTable().draw("page");
...
function notty(data,type){
	...
	if (type=="notification") { var timeout = "1000"; } else if(type !== false) { var timeout = "10000";/*false;*/ } else { type = 'error' }
 	noty({
		text: msg,
		layout      :	'topCenter',
		type        :	type,
